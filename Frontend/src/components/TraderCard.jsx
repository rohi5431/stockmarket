...
const TraderCard = ({ trader, index }) => {
  ...
  const toggleFollow = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Error updating follow status");
  ...
  // Pick a unique light background color based on index
  const bgColor = lightColors[index % lightColors.length];

  return (
    <TraderBox className={`relative w-full h-[26rem] ${bgColor} text-gray-900 rounded-2xl shadow-lg overflow-hidden`}>
      <div className="absolute top-4 left-4 text-blue-600 text-2xl font-extrabold">
        {trader.rank ? `#${trader.rank}` : ""}
      </div>

      <TraderBody className="flex flex-col items-center justify-center h-full text-center space-y-4">
        <div className="w-20 h-20 flex items-center justify-center rounded-full bg-blue-100 text-blue-800 text-2xl font-bold">
          {trader.name.charAt(0)}
        </div>

        <h3 className="text-xl font-semibold">{trader.name}</h3>
        <p className="text-sm text-gray-500">{trader.symbol}</p>

        <p className="text-green-600 font-bold text-lg">{trader.roi}</p>
        <p className="text-sm text-gray-600">{trader.return}</p>
